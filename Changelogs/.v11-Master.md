...
* **Powah! (Rearchitected)**
    * Description: Re-added this powerful tech mod focused on energy generation, storage, and transfer.
    * CurseForge: [https://www.curseforge.com/minecraft/mc-mods/powah-rearchitected](https://www.curseforge.com/minecraft/mc-mods/powah-rearchitected)
...
* **Torchmaster Functionality:** With both Torchmaster and its Cobblemon Compat addon installed, the **Mega Torch** block can be placed in your base (or elsewhere) to prevent both hostile mobs *and* wild Cobblemon from spawning naturally within a large radius (default 64x64 blocks).
    * *Configuration Note:* You might need to manually edit the `torchmaster.toml` config file and add `"+cobblemon:pokemon"` to the `megaTorchEntityBlockListOverrides` list for the Mega Torch to block Cobblemon spawns, depending on the mod's default settings.
